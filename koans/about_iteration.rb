...
class AboutIteration < Neo::Koan
  ...
  def test_inject_will_blow_your_mind
    result = [2, 3, 4].inject(0) { |sum, item| sum + item } # collector starts with 0
    assert_equal 9, result

    result2 = [2, 3, 4].inject(1) { |product, item| product * item } # collector starts with 1
    assert_equal 24, result2

    # Extra Credit:
    # Describe in your own words what inject does.
    # "collects" the result as specified in the block
  end
  ...
  #   # code to read 'file'
  # proposed reply: not to have to close it
  #
